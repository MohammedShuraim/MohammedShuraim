"""
Ultimate GitHub Banner Generator v2:
- Replicates arifhaxn's exact terminal window layout & glowing cyan corner brackets
- macOS traffic light window control buttons (Red, Yellow, Green)
- Full-density Floyd-Steinberg dithered portrait (Layer 1) - visible instantly on load
- Optimal Transport SMIL morphing travellers (Layer 2) across 2 crisp high-contrast logos:
  1. Sharingan / Rinnegan Eye (Naruto)
  2. One Piece Strawhat Jolly Roger
- Theme-aware palette matching arifhaxn (#A78BFA portrait, #22D3EE UI chrome, #7C3AED email pill)
"""
import os
import random
import numpy as np
...
SHARINGAN_PATH = os.path.join(BRAIN_USER, "media__1785340694010.png")
ONEPIECE_PATH = os.path.join(BRAIN_USER, "media__1785340694107.png")
...
def extract_logo_points(img_path, num_points=950, target_w=290, target_h=350):
...
def generate_svg(dot_matrix, logo1_pts, logo2_pts, palette, is_dark, output_path):
    ...
    # Glowing Corner Brackets (matching arifhaxn's screenshot!)
    ...
    # Timing (12.0s loop):
    # t=0s -> 0.000: fully visible (opacity 1)
    # t=3.0s -> 0.250: start dissolve
    # t=4.2s -> 0.350: fully dissolved (opacity 0)
    # t=10.8s -> 0.900: start return
    # t=12.0s -> 1.000: fully returned (opacity 1)
    
    keytimes_l1 = "0; 0.250; 0.350; 0.900; 1"
    ...
    # --- Layer 2: Travellers (Morphing across 2 Anime/Manga Logos) ---
    # Timing (12.0s loop):
    # 0s -> 3.0s (0.250): hidden (opacity 0)
    # 4.2s (0.350): Form Logo 1 (Sharingan Eye) - opacity 1
    # 6.6s (0.550): Hold Logo 1 -> morph to Logo 2 (One Piece Jolly Roger)
    # 7.8s (0.650): Form Logo 2 (One Piece Jolly Roger)
    # 10.2s (0.850): Hold Logo 2 -> return to center
    # 10.8s (0.900): fade out (opacity 0)
    # 12.0s (1.000): hidden
    
    keytimes_l2 = "0; 0.250; 0.350; 0.550; 0.650; 0.850; 0.900; 1"
    opacity_l2 = "0; 0; 0.95; 0.95; 0.95; 0.95; 0; 0"
    ...
    for i in range(len(logo1_pts)):
        ...
        cx_vals = f"{cx0}; {cx0}; {x1}; {x1}; {x2}; {x2}; {cx0}; {cx0}"
        cy_vals = f"{cy0}; {cy0}; {y1}; {y1}; {y2}; {y2}; {cy0}; {cy0}"
...
def main():
    print("[v2] Extracting logo points...")
    pts1 = extract_logo_points(SHARINGAN_PATH, num_points=NUM_TRAVELLERS)
    pts2_raw = extract_logo_points(ONEPIECE_PATH, num_points=NUM_TRAVELLERS)
    
    print("[v2] Running Hungarian optimal transport point alignment...")
    pts2 = align_points(pts1, pts2_raw)
    
    print("[v2] Processing portrait photo...")
    ...
    print("[v2] Building dark.svg...")
    generate_svg(dark_portrait, pts1, pts2, palettes['dark'], True, os.path.join(ROOT, "dark.svg"))
    
    print("[v2] Building light.svg...")
    generate_svg(light_portrait, pts1, pts2, palettes['light'], False, os.path.join(ROOT, "light.svg"))
