"""
Master Banner Generator with Bold, Dense Particles & Famous Logos:
- 1200 Dense, Bold Particles (radius 2.2px / stroke 2.0px) for rich visibility (NOT thin pencil lines)
- Logos:
  1. Girigo / Praying Hands 'A' Emblem (User Uploaded Image)
  2. Python 🐍 Logo
  3. </ > Code Glyph 💻
- Hungarian Optimal Transport alignment
- Full-density Floyd-Steinberg portrait (Layer 1) visible instantly on load
...
def generate_python_logo(num_points=1200, w=290, h=350):
    cx, cy = w / 2, h / 2
    pts = []
    n_loop = num_points // 2
    for t in np.linspace(0, 2*math.pi, n_loop):
        x = cx - 20 + 52 * math.cos(t) * abs(math.cos(t))**0.3
        y = cy - 30 + 52 * math.sin(t) * abs(math.sin(t))**0.3
        pts.append((x, y))
...
def generate_svg(dot_matrix, logo1_pts, logo2_pts, logo3_pts, palette, is_dark, output_path):
    ...
    # --- Layer 1: Portrait Bands (Stroke Width = 1.8 for BOLD high density!) ---
    ...
    # --- Layer 2: Travellers (1200 Bold Dots r="2.0" morphing across 3 Famous Logos: Girigo -> Python -> Code) ---
    keytimes_l2 = "0; 0.211; 0.303; 0.444; 0.535; 0.676; 0.768; 0.908; 1"
    opacity_l2 = "0; 0; 0.98; 0.98; 0.98; 0.98; 0.98; 0; 0"
...
def main():
    print("Generating bold 1200-particle logos (Girigo A-Emblem, Python, Code)...")
    pts1 = extract_girigo_points(GIRIGO_PATH, num_points=NUM_TRAVELLERS)
    pts2_raw = generate_python_logo(num_points=NUM_TRAVELLERS)
    pts3_raw = generate_code_glyph(num_points=NUM_TRAVELLERS)
